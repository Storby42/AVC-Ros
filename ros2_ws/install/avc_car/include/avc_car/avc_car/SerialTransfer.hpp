    ...
    class PacketCRC
    {
        public: // <<---------------------------------------//public
            ...
            PacketCRC(const uint8_t& polynomial = 0x9B, const uint8_t& crcLen = 8)
            {
                poly      = polynomial;
                crcLen_   = crcLen;
                tableLen_ = static_cast<uint16_t>(1u << crcLen); // avoid pow() returning double
        ...
        public: // <<---------------------------------------//public
            ...
            */
            uint8_t constructPacket(const uint16_t& messageLen, const uint8_t& packetID = 0)
            {
                ...
                {
                    calcOverhead(txBuff, static_cast<uint8_t>(messageLen));
                    stuffPacket(txBuff, static_cast<uint8_t>(messageLen));
                    uint8_t crcVal = crc.calculate(txBuff, static_cast<uint8_t>(messageLen));

                    preamble[1] = packetID;
                    preamble[2] = overheadByte;
                    preamble[3] = static_cast<uint8_t>(messageLen);

                    postamble[0] = crcVal;

                    return static_cast<uint8_t>(messageLen);
                }
            };
            /*
            ...
            void stuffPacket(uint8_t arr[], const uint8_t& len)
            {
                ...
                if (refByte != -1)
                {
                    for (uint8_t i = (len - 1); i != 0xFF; i--)
                    {
                        if (arr[i] == START_BYTE)
                        {
                            arr[i]  = static_cast<uint8_t>(refByte - i);
                            refByte = i;
                        }
                    }
                }
