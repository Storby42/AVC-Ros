...
*avc_car* is a simple mobile base using bicycle model with 4 wheels.
...
1. To check that *avc_car* description is working properly use following launch commands

   .. code-block:: shell

    ros2 launch avc_car view_robot.launch.py
...
2. To start *avc_car* example open a terminal, source your ROS2-workspace and execute its launch file with

   .. code-block:: shell

    ros2 launch avc_car avc_car.launch.py remap_odometry_tf:=true
...
3. Now, let's introspect the control system before moving *avc_car*. Check if the hardware interface loaded properly, by opening another terminal and executing
   ...
   The ``[claimed]`` marker on command interfaces means that a controller has access to command *avc_car*.
...
5. If everything is fine, now you can send a command to *bicycle_steering_controller* using ROS 2 CLI:
   ...
   .. code-block:: shell
      ...
      [ros2_control_node-1] [INFO] [1721766165.108212153] [controller_manager.resource_manager.hardware_component.system.avc_car]: Writing commands:
...
* Launch file: `avc_car.launch.py <https://github.com/ros-controls/ros2_control_demos/tree/{REPOS_FILE_BRANCH}/example_11/bringup/launch/avc_car.launch.py>`__
